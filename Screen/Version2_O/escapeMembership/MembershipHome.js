...
export default function MembershipHome() {
    ...
    const totalAmtFromBackend = (membershipAmt + platformFeeFromBackend + gstFromBackend) || 0;
    ...
    const handleInitiatePayment = async () => {
        try {
            ...
            const payload = JSON.stringify({
                ...
                investmentPlanId: selectedPlan?._id || investmentData[0]?._id,
                phoneNumber: globalState?.userDetails?.phoneNumber,
                amount: selectedPlan?.offer ? totalAmtFromBackend : totalAmt,
                // amount: 1,
                // totalAmount: 1,
                            ...
                            >
                                <ScrollView showsVerticalScrollIndicator={false}>
                                    ...
                                    <View style={{ marginTop: 15 }}>
                                        ...
                                        <View style={{ borderColor: '#00000066', borderWidth: 0.5, borderRadius: 5, marginTop: 10}}>
                                            <TextInput
                                                placeholder='John Doe'
                                                placeholderTextColor={'#00000066'}
                                                value={name}
                                                onChangeText={setName}
                                                style={{ marginVertical: -5, fontFamily: 'WorkSans-Regular', fontSize: 12, color: '#000', marginLeft:10 }}
                                            />
                                        </View>
                                    </View>
                                ...
                                <View style={{ marginTop: 15 }}>
                                    ...
                                    <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                                        ...
                                        {!selectedPlan?.offer ?
                                            <Text style={{ fontFamily: 'WorkSans-SemiBold', fontSize: 14, color: '#000' }}>₹{formatIndianAmount(totalAmt)}</Text>
                                            :
                                            <Text style={{ fontFamily: 'WorkSans-SemiBold', fontSize: 14, color: '#000' }}>₹{formatIndianAmount(totalAmtFromBackend)}</Text>
                                        }
                                    </View>
